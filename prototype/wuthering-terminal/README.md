...
- **Unit snapshots**: each `Snapshot` lists combatants with name, team, tile position, HP/AP, selection, elemental status, remaining Rooted/Stunned durations, and any elemental shield so scripts and agents plan from structured state instead of scraping frames or HUD badges. `reachable_tiles` and `targetable_tiles` are computed from the selected unit's tile (class attack range), not the cursor.
