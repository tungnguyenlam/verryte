...
for f in sorted(os.listdir(ASSETS)):
    if f.startswith("rover") and f.endswith(".png"):
        rover_path = os.path.join(ASSETS, f)
        break
...
for tier in TIERS:
    ...
    print(f"  [{fits}] {t_label:8s}  {t_sw:2d}×{t_sh:2d} sub-px  ({t_sw}×{t_sh//2} cells)  min {t_min_c}×{t_min_r}{sel}")

# Render the selected tier
print(f"\n--- ROVER at {label} resolution ({sw}×{sh} → {sw}×{sh//2} cells) ---")
result = png_to_ansi(rover_path, sw, sh)
if result:
    print(result)
...
if tier_idx > 0:
    ...
    r = png_to_ansi(rover_path, prev[1], prev[2])
    if r: print(r)

if tier_idx < len(TIERS) - 1:
    nxt = TIERS[tier_idx + 1]
    print(f"\n--- (one tier up: {nxt[0]} {nxt[1]}×{nxt[2]}) ---")
    r = png_to_ansi(rover_path, nxt[1], nxt[2])
    if r: print(r)
